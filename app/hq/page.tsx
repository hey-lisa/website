...
export const metadata: Metadata = {
  title: "HQ | Hey LiSA",
  description: "Updates and thoughts from the team.",
};

export default async function HQIndexPage() {
  const posts = (await getAllHQFrontmatter()).sort(
  ...
  return (
    <div className="flex flex-col gap-1 pt-2">
      <div className="grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 sm:gap-8 gap-4 mb-5">
        {posts.map((post) => (
          <HQCard {...post} slug={post.slug} key={post.slug} />
        ))}
      </div>
    </div>
  );
...
function HQCard({
  ...
  cover,
  authors,
}: HQMdxFrontmatter & { slug: string }) {
  return (
    <Link
      href={`/hq/${slug}`}
      className="flex flex-col gap-2 items-start border rounded-md py-5 px-3 min-h-[400px] transition-all duration-300 blog-card-hover"
    >
