...
export default function SubLink({
  ...
  const Comp = (
    ...
    >
      {title}
      {tag && (
        <span className="bg-blue-700 rounded-md px-1.5 py-0.5 mx-2 text-xs text-white !font-normal">
          {tag}
        </span>
      )}
    </Anchor>
    ...
    <div className="flex flex-col gap-1 w-full">
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        ...
        <CollapsibleContent>
          <div
            className={cn(
              "flex flex-col items-start sm:text-sm text-stone-300/85 ml-0.5 mt-2.5 gap-3",
